...
from dali.loader import TrainPipeline, TestPipeline
...
def get_dataset_dali(dset_name, batch_size, n_worker, data_root='../../data'):
    cifar_tran_train = [
    ...
    if dset_name == 'cifar10':
        ...
        trainpipe = TrainPipeline(batch_size=batch_size, num_threads=6, device_id = 0, img_root = traindir, shuffle=True)
        trainpipe.build()
        train_loader = DALIGenericIterator(trainpipe, ['data', 'label'], trainpipe.epoch_size("TrainReader"))

        testpipe = TestPipeline(batch_size=batch_size, num_threads=6, device_id = 0, img_root = valdir, shuffle=False)
        testpipe.build()
        val_loader = DALIGenericIterator(testpipe, ['data', 'label'], testpipe.epoch_size("TestReader"))
